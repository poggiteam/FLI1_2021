---
title: "03_2020-42set1et2Seurat_analysis_hto_souporcell_run_février_2022"
...
# knitr::opts_knit$set(root.dir = "/media/bigott/DATA/01_tim/01_these/01_etv6/06_Analysis/")
# ```
...
OUTPUT_PATH <- "/home/bigott/workspace2/DATA/01_tim/01_these/01_etv6/05_Outputs/06_Février_2022/03_2020-42-set1et2/"

# Set the random number seed
set.seed(1234)

# Load path for files
PATH_DATA_J6 <- "/home/bigott/workspace2/01_tim/01_these/01_etv6/05_Outputs/06_Février_2022/01_2020-42-set1/"
PATH_DATA_J11 <- "/home/bigott/workspace2/01_tim/01_these/01_etv6/05_Outputs/06_Février_2022/02_2020-42-set2/"
...
top100 <- head(VariableFeatures(ETV6_2020), 100)
```
...
ctrl.object <- ScaleData(ctrl.object, features = all.genes, display.progress = TRUE, do.scale=FALSE, vars.to.regress = c("S.Score","G2M.Score"))
```
...
########################################
##top 10 DE gene expression by cluster##
########################################
...
Idents(ctrl.object) <- "RNA_snn_res.1.2"
ctrl.object@active.ident <- factor(ctrl.object@active.ident, levels = c("14","12","8","3","6","1","2","0","4","5","9","11","7","10","13"))
# dp <- DotPlot(ctrl.object, features = c("KIT","THY1","SLAMF1","NPR3","AVP","PROM1","HOPX","FLT3","CRHBP","SPINK2","CSF3R","IGHM","CD34","SMIM24","DEPTOR","HACD1","CPB1","SVOPL","CD38","GLIPR1","TFRC","TGFB1","PLEK","TRPC6","GP6","ITGB3","SELP","VWF","ITGA2B","LTBP1","GP9","MEIS1","MPIG6B","PPBP","GP1BA","PF4","RASGRP2","TFR2","ANK1","ACSM3","KLF1","HBB","EPOR","MPO","ELANE","SERPINB10","CTSG","CPA3","MS4A2","TPSAB1")) + geom_point(aes(size=pct.exp), shape = 21, colour="black", stroke=0.5) + scale_colour_gradient2(low = "steelblue", mid = "ivory1", high = "red") + guides(size=guide_legend(override.aes=list(shape=21, colour="black", fill="white"))) + RotatedAxis()
# dp

dp <- DotPlot(ctrl.object, features = c("THY1","PROM1","FLT3","CRHBP","HOPX","AVP","KIT","SPINK2","CSF3R","MPO","ELANE","PRTN3","AZU1","SERPINB10","CTSG","CPA3","CLC","PRG2","TPSAB1","VWA5A","CD38","TFRC","DEPTOR","KLF1","TFR2","HBB","APOC1","ANK1","ACSM3","LMNA","CAVIN2","LAT","CD9","STOM","PLEK","ITGA2B","GP9","GP6","GP1BA","PF4","RASGRP2","MPIG6B","VWF","PPBP","SELP","LTBP1")) + geom_point(aes(size=pct.exp), shape = 21, colour="black", stroke=0.5) + scale_colour_gradient2(low = "steelblue", mid = "ivory1", high = "red") + guides(size=guide_legend(override.aes=list(shape=21, colour="black", fill="white"))) + RotatedAxis()
dp
```

## Dimensional reduction plot, with cells colored by a quantitative feature
```{r echo=FALSE}
gene_list <- c("PROM1","SPINK2","CD34","KIT","TFRC","PLEK","ITGB3","VWF","ITGA2B","GP9","KLF1","HBB","MPO","SERPINB10","TPSAB1","CPA3","PPBP")


for (i in 1:length(gene_list)) {
    tryCatch({
      print(gene_list[i])
      print(FeaturePlot(object = ctrl.object, features = gene_list[i], reduction = "umap", cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black"), pt.size = 1)+NoAxes()+NoLegend())
    }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
```

## Feature Plot signatures {.tabset}
Adding module score (based on published signatures) in meta.data and plotting it with FeaturePlot. Other solutions exist for signatures and shall be considered.

```{r FP_sig, echo=FALSE, fig.width=10,fig.height=8}
ctrl.object <- AddModuleScore(ctrl.object, features = list(c("PROM1","FLT3","CRHBP","HOPX","AVP","IGHM","GLIPR1")),
name = "HSPC")

ctrl.object <- AddModuleScore(ctrl.object, features = list(c("CPA3","PRG2","RHEX","ALOX5AP","MS4A2")),
name = "CMP")
ctrl.object <- AddModuleScore(ctrl.object, features = list(c("MS4A3","RNASE2","EPX","CLC","CSF2RB","CTSG")),
name = "non.primed.CMP")
ctrl.object <- AddModuleScore(ctrl.object, features = list(c("LMNA","CAVIN2","LAT","VWA5A","KIT","HPGDS","KRT1","TPSB2","TPSAB1","TMEM176B","SAMSN1","CD44")),
name = "Mk.primed.CMP")

ctrl.object <- AddModuleScore(ctrl.object, features = list(c("CSTA","MPO","ELANE","PRTN3","AZU1","RNASE3","CFD")),
name = "GMP")

ctrl.object <- AddModuleScore(ctrl.object, features = list(c("CD38","KLF1","DEPTOR","ACSM3","APOC1","TFRC","ANK1","TMSB10","ALDH1A1","MT-ATP8","H2AFY","GYPB")),
name = "MEP")
ctrl.object <- AddModuleScore(ctrl.object, features = list(c("BLVRB","HBB","APOC1","TFR2")),
name = "MEP_ERP")

ctrl.object <- AddModuleScore(ctrl.object, features = list(c("MPIG6B","PF4","GP9","VWF","PPBP","SH3BP5","SELP","LTBP1","TMEM40")),
name = "MkP_Mk")
```

### HSPC
```{r FP_sig1, echo=FALSE, fig.width=10,fig.height=8}
FeaturePlot(object = ctrl.object, features = "HSPC1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()
```

### CMP
```{r FP_sig2, echo=FALSE, fig.width=10,fig.height=8}
FeaturePlot(object = ctrl.object, features = "CMP1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()

FeaturePlot(object = ctrl.object, features = "Mk.primed.CMP1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()

FeaturePlot(object = ctrl.object, features = "non.primed.CMP1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()
```

### GMP
```{r FP_sig3, echo=FALSE, fig.width=10,fig.height=8}
FeaturePlot(object = ctrl.object, features = "GMP1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()
```

### MEP
```{r FP_sig4, echo=FALSE, fig.width=10,fig.height=8}
FeaturePlot(object = ctrl.object, features = "MEP1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()

FeaturePlot(object = ctrl.object, features = "MEP_ERP1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()
```

### MkP.Mk
```{r FP_sig6, echo=FALSE, fig.width=10,fig.height=8}
FeaturePlot(object = ctrl.object, features = "MkP_Mk1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()
```
...
for (i in 1:length(rownames(ctrl.object@meta.data))) {
	ifelse(ctrl.object@meta.data$RNA_snn_res.1[i] %in% "11",ctrl.object@meta.data$celltypes[i] <- "GMP",
		ifelse(ctrl.object@meta.data$RNA_snn_res.1[i] %in% "8", ctrl.object@meta.data$celltypes[i] <- "HSPC",
			ifelse(ctrl.object@meta.data$RNA_snn_res.1[i] %in% "3", ctrl.object@meta.data$celltypes[i] <- "CMP",
				ifelse(ctrl.object@meta.data$RNA_snn_res.1[i] %in% "9", ctrl.object@meta.data$celltypes[i] <- "CMP-Mkprimed",
					ifelse(ctrl.object@meta.data$RNA_snn_res.1[i] %in% "4", ctrl.object@meta.data$celltypes[i] <- "MEP-ERP",
						ifelse(ctrl.object@meta.data$RNA_snn_res.1[i] %in% "7", ctrl.object@meta.data$celltypes[i] <- "MEP",
							ifelse(ctrl.object@meta.data$RNA_snn_res.1[i] %in% "2", ctrl.object@meta.data$celltypes[i] <- "MEP-MkP",
								ifelse(ctrl.object@meta.data$RNA_snn_res.1[i] %in% "1", ctrl.object@meta.data$celltypes[i] <- "MkP",
									ifelse(ctrl.object@meta.data$RNA_snn_res.1[i] %in% "6", ctrl.object@meta.data$celltypes[i] <- "MkP-Mk",
										ifelse(ctrl.object@meta.data$RNA_snn_res.1[i] %in% "5", ctrl.object@meta.data$celltypes[i] <- "MkP-Mk",
										  ifelse(ctrl.object@meta.data$RNA_snn_res.1[i] %in% "0", ctrl.object@meta.data$celltypes[i] <- "MEP",
										    ctrl.object@meta.data$celltypes[i] <- "MkP-Mk")))))))))))
}

DimPlot(ctrl.object, group.by = "celltypes")
```
...
top100 <- head(VariableFeatures(pat.object), 100)
```
...
for(i in 0:(max(as.numeric(levels(pat.object@meta.data$seurat_clusters))))) {
  assign(paste("cluster", i,".markers", sep = ""), FindMarkers(pat.object, features = all.genes_without_ribosomal, ident.1 = i, test.use = "bimod", only.pos = TRUE))
    print(paste("cluster", i,".markers", sep = ""))
    print(head(get(paste("cluster", i,".markers", sep = "")),n=50))
    write.table(get(paste("cluster", i,".markers", sep = "")),
		paste("cluster", i,"markers_pat_object.txt", sep = "_"),
		quote=FALSE,row.names = TRUE)
}
```
...
```{r dotplot_supervised, echo=FALSE, message=FALSE,fig.width=10,fig.height=8}
Idents(pat.object) <- "RNA_snn_res.0.8"
pat.object@active.ident <- factor(pat.object@active.ident, levels = c("1","0","8","6","2","5","4","10","7","3","9"))

dp <- DotPlot(pat.object, features = c("THY1","PROM1","FLT3","CRHBP","HOPX","AVP","KIT","SPINK2","CSF3R","CPA3","CLC","PRG2","TPSAB1","VWA5A","MPO","ELANE","PRTN3","AZU1","SERPINB10","CTSG","CD38","TFRC","DEPTOR","KLF1","TFR2","HBB","APOC1","ANK1","ACSM3","LMNA","CAVIN2","LAT","CD9","STOM","PLEK","ITGA2B","GP9","GP6","GP1BA","PF4","RASGRP2","MPIG6B","VWF","PPBP","SELP","LTBP1")) + geom_point(aes(size=pct.exp), shape = 21, colour="black", stroke=0.5) + scale_colour_gradient2(low = "steelblue", mid = "ivory1", high = "red") + guides(size=guide_legend(override.aes=list(shape=21, colour="black", fill="white"))) + RotatedAxis()
dp
```
...
```{r FP_sig, echo=FALSE, fig.width=10,fig.height=8}
pat.object <- AddModuleScore(pat.object, features = list(c("PROM1","FLT3","CRHBP","HOPX","AVP","IGHM","GLIPR1")),
name = "HSPC")

pat.object <- AddModuleScore(pat.object, features = list(c("CPA3","PRG2","RHEX","ALOX5AP","MS4A2")),
name = "CMP")
pat.object <- AddModuleScore(pat.object, features = list(c("MS4A3","RNASE2","EPX","CLC","CSF2RB","CTSG")),
name = "non.primed.CMP")
pat.object <- AddModuleScore(pat.object, features = list(c("LMNA","CAVIN2","LAT","VWA5A","KIT","HPGDS","KRT1","TPSB2","TPSAB1","TMEM176B","SAMSN1","CD44")),
name = "Mk.primed.CMP")

pat.object <- AddModuleScore(pat.object, features = list(c("CSTA","MPO","ELANE","PRTN3","AZU1","RNASE3","CFD")),
name = "GMP")

pat.object <- AddModuleScore(pat.object, features = list(c("CD38","KLF1","DEPTOR","ACSM3","APOC1","TFRC","ANK1","TMSB10","ALDH1A1","MT-ATP8","H2AFY","GYPB")),
name = "MEP")
pat.object <- AddModuleScore(pat.object, features = list(c("BLVRB","HBB","APOC1","TFR2")),
name = "MEP_ERP")

pat.object <- AddModuleScore(pat.object, features = list(c("MPIG6B","PF4","GP9","VWF","PPBP","SH3BP5","SELP","LTBP1","TMEM40")),
name = "MkP_Mk")
```

### HSPC
```{r FP_sig1, echo=FALSE, fig.width=10,fig.height=8}
FeaturePlot(object = pat.object, features = "HSPC1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()
```

### CMP
```{r FP_sig2, echo=FALSE, fig.width=10,fig.height=8}
FeaturePlot(object = pat.object, features = "CMP1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()

FeaturePlot(object = pat.object, features = "Mk.primed.CMP1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()

FeaturePlot(object = pat.object, features = "non.primed.CMP1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()
```

### GMP
```{r FP_sig3, echo=FALSE, fig.width=10,fig.height=8}
FeaturePlot(object = pat.object, features = "GMP1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()
```

### MEP
```{r FP_sig4, echo=FALSE, fig.width=10,fig.height=8}
FeaturePlot(object = pat.object, features = "MEP1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()

FeaturePlot(object = pat.object, features = "MEP_ERP1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()
```

### MkP.Mk
```{r FP_sig6, echo=FALSE, fig.width=10,fig.height=8}
FeaturePlot(object = pat.object, features = "MkP_Mk1", reduction = "umap", order = TRUE,
pt.size = 0.6, cols = c("grey","lightblue","cyan3","cyan4","dodgerblue3","blue","mediumslateblue","purple","orchid3","red","brown","black")) + NoAxes() +NoLegend()
```
...
for (i in 1:length(rownames(pat.object@meta.data))){
    if (pat.object@meta.data$RNA_snn_res.2[i] %in% 1){
      pat.object@meta.data$celltypes[i] <- "MkP_Mk"
      }
    if (pat.object@meta.data$RNA_snn_res.2[i] %in% c(0,8,6,2,5)){
      pat.object@meta.data$celltypes[i] <- "MEP"
      }
    if (pat.object@meta.data$RNA_snn_res.2[i] %in% 4){
      pat.object@meta.data$celltypes[i] <- "CMP"
      }
    if (pat.object@meta.data$RNA_snn_res.2[i] %in% 10){
      pat.object@meta.data$celltypes[i] <- "CMP_Primed"
      }
    if (pat.object@meta.data$RNA_snn_res.2[i] %in% 9){
      pat.object@meta.data$celltypes[i] <- "GMP"
      }
    if (pat.object@meta.data$RNA_snn_res.2[i] %in% c(3,7)){
