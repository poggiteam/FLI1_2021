...
CWD <- "/media/station/DATA/01_tim/01_these/01_etv6/02_scripts/" 
#  Path to the folder that will contain output objects
OUTPUT_PATH <- "/media/station/DATA/01_tim/01_these/01_etv6/04_pseudotime_data/Objets_R_Demux/last_but_not_least/"
...
# Load path for files
PATH_HUMAN_DATA1 <- "/media/station/DATA/01_tim/01_these/00_Fli1/00_data/00_sets_meso/01_run325/FLI1_run325_GRCh382020A_CR4/outs/filtered_feature_bc_matrix/"
# SAMPLE1 <- "D6"
PATH_HTO_DATA1 <- "/media/station/DATA/01_tim/01_these/00_Fli1/00_data/00_sets_meso/01_run325/CITE-seq-count143_FLI1_run325__HTO_nofilter-wl_Result_hd2/umi_count/"
```
## Data preprocessing
...
```{r}
all_clusters <- read.csv("/media/station/DATA/01_tim/01_these/02_Souporcell/08_FLIrun_325/clusters.tsv", header = T, sep = "\t")
...
all_clusters[all_clusters$barcode == c("TCATATCTCTAGTGTG-1"),]
```
...
```{r}
  ...
  Seurat5 <- QC_function_mito_threshold(Seurat = clean.subset1, mito_threshold = 0.1, do_plot = TRUE)
```

```{r}
  #2- Find variable genes
  Seurat5 <- FindVariableFeatures(object = Seurat5, 
                            assay = "RNA", selection.method = "vst", nfeatures = 2000,
                            verbose = FALSE)
  Seurat5 <- ScaleData(Seurat5, 
                      assay="RNA",
                      verbose = FALSE,
                      features = rownames(Seurat5),
                      do.center = TRUE)
  Seurat5 <- RunPCA(object = Seurat5,
                   assay = "RNA",
                   verbose = FALSE,
                   features =  VariableFeatures(object = Seurat5),
                   seed.use = 1234,
                   npcs = 50)
  ElbowPlot(Seurat5, ndims = 50, reduction = "pca")
```
# Scorer les genes pour les composantes
```{r}
  Seurat5 <- ProjectDim(object = Seurat5,
                  nfeatures.print = 10,
                  dims.print = 1:20)
  Seurat5 <- RunTSNE(object = Seurat5,
                  do.fast = TRUE, 
                  seed.use = 1234,
                  dims = 1:20,
                  perplexity = 40)
  Seurat5 <- FindNeighbors(object = Seurat5, 
                  dims = 1:20, 
                  verbose = FALSE, 
                  force.recalc = TRUE, 
                  reduction = "pca",
                  k.param = 30)
  Seurat5 <- FindClusters(object = Seurat5, 
                  resolution = 0.5,
                  verbose = T,
                  random.seed = 1234)
  Seurat5 <- RunUMAP(object = Seurat5, reduction = "pca", seed.use = 1234, dims = 1:20)
save(Seurat5, file = paste0(OUTPUT_PATH, "07_run325", ".Robj"))
```
###UMAPs
```{r tSNE_HTO_seurat_1}
ggplotly(DimPlot(Seurat5, reduction = "umap", label = F, pt.size = 1, ncol = 2, group.by = "souporcell_classif", split.by = "orig.ident"))
```
```{r}
ggplotly(DimPlot(Seurat5, reduction = "umap", group.by = "souporcell_classif", label = F, pt.size = 1, ncol = 2))
table(Seurat5@meta.data$souporcell_classif)
```
```{r tSNE_HTO_seurat_1}
...
```{r}
Seurat5@meta.data$identity <- "NA"
Idents(Seurat5) <- "souporcell_classif"
Seurat5@meta.data[Seurat5@meta.data$souporcell_classif == "cluster0",]$identity = "Ctrl2"
Seurat5@meta.data[Seurat5@meta.data$souporcell_classif == "cluster2",]$identity = "Ctrl1"
Seurat5@meta.data[Seurat5@meta.data$souporcell_classif == "cluster1",]$identity = "K345E.II.1"
# Seurat5@meta.data[Seurat5@meta.data$souporcell_classif == "cluster3",]$identity = "j6F417.II.2"
ggplotly(DimPlot(Seurat5, pt.size = 1, ncol = 2, reduction = "umap", group.by = "identity"))
```
```{r}
table(Seurat5@meta.data$identity,Seurat5@meta.data$souporcell_classif)
```
# ETV6status
```{r}
Seurat5@meta.data$ETV6status <- "NA"
Idents(Seurat5) <- "identity"
Seurat5@meta.data[Seurat5@meta.data$identity == "Ctrl1",]$ETV6status = "01_Témoins"
Seurat5@meta.data[Seurat5@meta.data$identity == "Ctrl2",]$ETV6status = "01_Témoins"
Seurat5@meta.data[Seurat5@meta.data$identity == "K345E.II.1",]$ETV6status = "02_Patient"
# Seurat5@meta.data[Seurat5@meta.data$identity == "j6P214.III.8",]$ETV6status = "02_Patients"
ggplotly(DimPlot(Seurat5, pt.size = 1, ncol = 2, reduction = "umap", group.by = "ETV6status"))
```
```{r}
table(Seurat5@meta.data$ETV6status,Seurat5@meta.data$souporcell_classif)
```

# orig.ident
```{r}
Seurat5@meta.data$orig.ident <- "NA"
Seurat5@meta.data[rownames(Seurat5@meta.data),]$orig.ident = "01_J5"
ggplotly(DimPlot(Seurat5, pt.size = 1, ncol = 2, reduction = "umap", group.by = "orig.ident"))
```
```{r}
table(Seurat5@meta.data$orig.ident)
```
```{r}
save(Seurat5, file = paste0(OUTPUT_PATH, "07_run325", ".Robj"))
```
...
```{r mito_vs_nfeatures}
df<-data.frame(identity=Seurat5@misc$old_meta_data$souporcell_classif,percent.mito=Seurat5@misc$old_meta_data$percent.mito,nFeature_RNA=Seurat5@misc$old_meta_data$nFeature_RNA)
